...
class ListBooks:
    ...
    def restore_pickle(self, file):
        if dic_booklist := Util.restore_pickle(file):
            ...
            if not os.path.isdir(self.path):
                self.path = Code.configuration.paths.folder_userdata()
    ...
    def save_pickle(self, file):
        dic = {
            "path": self.path,
            "_modoAnalisis": self._modoAnalisis,
            "lista": [book.to_dic() for book in self.lista],
        }
        Util.save_pickle(file, dic)

    def restore(self):
        self.restore_pickle(Code.configuration.paths.file_books())
        self.verify()

    def save(self):
        self.save_pickle(Code.configuration.paths.file_books())
    ...
    def at_least_one(self):
        ...
        engine_rodent = Code.configuration.engines.search("rodentii")
