...
class IndexPolyglot:
    def __init__(self):
        self.index = Code.configuration.paths.file_index_polyglots()
        self.folder = Code.configuration.paths.folder_polyglots_factory()
            ...
            if not ok:
                with DBPolyglot(entry_file.path) as db:
                    d = {
                        ...
                        "MTIME": os.stat(entry_file.path).st_mtime,
                        "SIZE": len(db),
                    }
                    li_resp.append(d)
