...
class EngineResponse:
    def __init__(self, name: str, si_blancas: bool) -> None:
        self.name: str = name
        self.mate: int = 0
        self.puntos: int = 0
        self.without_movements: bool = False  # o por jaquemate o por ahogado
        self.pv: str = "a1a1"
        self.from_sq: str = ""
        self.to_sq: str = ""
        self.promotion: str = ""
    ...
    def change_side(self, position_before: Optional[Any] = None, mv_insert: Optional[str] = None) -> None:
        ...
        if mv_insert:
            self.pv = f"{mv_insert} {self.pv}"
            self.from_sq = mv_insert[:2]
    ...
    def texto_rival(self) -> str:
        if self.mate:
            ...
            if -1 < mate < 1:
                d = {
                    False: _("White is in checkmate"),
                    True: _("Black is in checkmate"),
                }
            else:
...
class MultiEngineResponse:
    ...
    def ordena(self):
        ...
        try:
            ...
            keys.sort()  # Fallback a orden lexicográfico si hay claves no numéricas
        for k in keys:
            rm = dic[k]
            mov = rm.movimiento()
    ...
    def check_pv(self, pv_base, is_bound):
        ...
        if "nodes" in d_claves:
            try:
                ...
                pass  # Keep previous nodes value
    ...
    def add_rm(self, rm):
        ...
        max_depth = 0
        included = False
        items = list(self.dicMultiPV.items())  # Crear lista para iterar seguro
        for cdepth, rm1 in items:
                ...
                if base_position.squares.get(first_move.from_sq, "p").lower() != "p":
                    if tipo == "F":
                        ...
                        has_advanced = base_position.proximity_middle(
                            side_engine
                        ) > position_after_move.proximity_middle(side_engine)
                    if has_advanced:
                        rm.puntos += x_apz
                        if dbg:
    ...
    def best_adjusted_move(self, n_tipo):
        ...
        if self.li_rm:
            ...
            if n_tipo in (
                ...
                nivel = {
                    ADJUST_WORSE: 1,
                    ADJUST_SOMEWHAT_WORSE_LESS: 2,
                    ADJUST_SOMEWHAT_WORSE_LESS_LESS: 3,
                }
                if not si_personalidad:
