...
class WVariations(QtWidgets.QWidget):
    def __init__(self, owner):
        ...
        ly_head = (
            Colocacion.H()
            .control(lb_variations)
            .relleno()
            .control(bt_mas)
            .control(bt_mas_engine)
            .control(bt_add_analysis)
            .control(bt_edit)
            .control(bt_remove)
            .margen(0)
        )
    ...
    def analyze_move(self, _num_move, num_variation, num_move_variation):
        ...
        Analysis.show_analysis(
            xanalyzer,
            move_var,
            self.get_board().is_white_bottom,
            num_move_variation,
            main_window=self,
        )
    ...
    def tb_add_analysis(self):
        ...
        manager = self.get_manager()
        xanalyzer = manager.manager_analyzer
    ...
    def tb_remove_variation(self):
        ...
        if num is None:
            ...
            if QTMessages.pregunta(self, f"{pgn}<br><br>{_('Are you sure you want to delete this line?')}"):
                ...
                if self.selected_link:
                    selected_link = (
                        self.selected_link.split("|")[0] if self.selected_link.count("|") == 2 else self.selected_link
                    )
                    self.link_variation_pressed(selected_link)
                self.get_manager().put_view()
