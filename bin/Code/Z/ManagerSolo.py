...
class ManagerSolo(Manager.Manager):
    ...
    def pon_rotulo(self):
        ...
        for label1, label2 in self.game.li_tags:
            ...
            if label1.upper() == "WHITE":
                li.append(f"{_('White')}: {label2}")
            elif label1.upper() == "BLACK":
                li.append(f"{_('Black')}: {label2}")
            elif label1.upper() == "RESULT":
                li.append(f"{_('Result')}: {label2}")
        mensaje = "\n".join(li)
    ...
    def end_game(self):
        ...
        if self.is_changed() and (len(self.game) > 0 or not self.game.is_fen_initial()):
            resp = QTMessages.question_withcancel(
                self.main_window,
                _("Do you want to save changes to a file?"),
                _("Yes"),
                _("No"),
            )
            if resp is None:
    ...
    def save_file(self, file):
        ...
        if Util.save_pickle(file, dic):
            ...
            QTMessages.message_error(self.main_window, f"{_('Unable to save')} : {file}")
            return False
    ...
    def file(self):
        ...
        if self.last_file:
            ...
            if os.curdir[:1] == rpath[:1]:
                ...
                if rpath.count("..") > 0:
                    rpath = self.last_file
            menuR.opcion("save", f"{_('Save')}: {rpath}", Iconos.Grabar())
            menuR.separador()
        ...
        if li:
            ...
            for path in li:
                menuR.opcion(f"reopen_{path}", Code.relative_root(path), Iconos.PuntoNaranja())
                menuR.separador()
        resp = menu.lanza()
        if resp is None:
            return
    ...
    def paste_position(self):
        ...
        if not ok:
            QTMessages.message_error(
                self.main_window,
                _("There is not a valid FEN position in the clipboard"),
            )
    ...
    def basic_initial_position(self):
        if len(self.game) > 0:
            if not QTMessages.pregunta(self.main_window, _("Do you want to remove all moves?")):
                return
        self.xfichero = None
        self.xpgn = None
        self.xjugadaInicial = None
    ...
    def control_teclado(self, nkey):
        if QTUtils.is_control_pressed():
            if nkey == QtCore.Qt.Key.Key_V:
                self.paste(QTUtils.get_txt_clipboard())
    ...
    def start_position(self):
        ...
        position, is_white_bottom = Voyager.voyager_position(self.main_window, self.game.first_position)
    ...
    def change_rival(self):
        ...
        if dic:
            ...
            if self.game.last_position.is_white != self.is_human_side_white and not self.game.is_finished():
                self.play_against_engine = False
                self.disable_all()
                self.play_rival()
                self.play_against_engine = True
